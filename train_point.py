...
from ldm.data.dataset_point import PointDataset
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                z = model.module.encode_first_stage((data['ori_img'] * 2 - 1.).to(device))
                z = model.module.get_first_stage_encoding(z)

            optimizer.zero_grad()
            model.zero_grad()
            features_adapter = model_ad(data['poi_img'].to(device))
